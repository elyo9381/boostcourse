...
* jDLC
	...
	sql 과 프로그래밍 언어의 통합 접근중 한 형태

  	* JDBC를 이용한 프로그래밍 방법
		- 1 단계 : imort java.sql.*;
		- 2 단계 : 드라이버를 로드한다.
		- 3 단계 : connection 객체를 생선한다.
		- 4 단계 : Statement 객체를 생성 및 질의 수행
		- 5 단계 : SQL문의 결과물이 있다면 ResultSet객체를 생성한다.
		- 6 단계 : 모든객체를 닫는다.
...
* API
	...
	API는 응용프로그램에서 사용할수 있도록, 운영체제나 프로그래밍 언어가 제공하는 기능을 제어할 수 있게 만든 인터페이스를 뜻 합니다.
	
	즉 운영체제나, 프로그래밍할 때 도움을 주는 메소드 혹은 라이브러리등이라고 말할수있다 (큰개념으로)

	Representational State Transfer 약자
	REST API는 REST 형식의 API를 말합니다.
	...
	* web api
		...
		post : post를 통햐 해당 url를 요청하면 리소스를 생성한다. 
...
* 복습하기
	자바스크립트 
	DOM : 무엇을 찾을떄
	Event : 홈페이지 눌렀을때 일처리 방법
	Ajax : 서버와 통신하는 방법 및 비동기 연결 
	함수선언 및 표현식 그리고 매개변수 
	함수를 익숙하고 자유롭게 사용해야한다. 
